...
<form action="<%= forgotPasswordURL %>" method="post" name="fm">
	...
	<!-- <aui:fieldset> -->
		...
		<%-- 		<aui:input label="<%= loginLabel %>" name="<%= loginParameter %>" size="30" type="text" value="<%= loginValue %>">
              ...
              <div class="formInnerField">
                ...
                <input type="text" name="<portlet:namespace/><%= loginParameter %>" value="<%= loginValue %>"  class="tetxBox emailonly" placeholder="" >
              </div>
            </div> 
				...
				<c:if test="<%= PropsValues.CAPTCHA_CHECK_PORTAL_SEND_PASSWORD %>">
					...
					<button type="submit" class="button button-fluid button-primary" value='<%= PropsValues.USERS_REMINDER_QUERIES_ENABLED ? "next" : "send-new-password" %>' >Send new Password </button>

			</c:when>
			<c:when test="<%= (user2 != null) && Validator.isNotNull(user2.getEmailAddress()) %>">
				<input name="step" type="hidden" value="2" />
				<input name="emailAddress" type="hidden" value="<%= user2.getEmailAddress() %>" />
				...
				<c:if test="<%= Validator.isNotNull(user2.getReminderQueryQuestion()) && Validator.isNotNull(user2.getReminderQueryAnswer()) %>">
					...
					<input autoFocus="<%= true %>" label="<%= HtmlUtil.escape(LanguageUtil.get(pageContext, user2.getReminderQueryQuestion())) %>" name="answer" type="text" />
				</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<!-- <aui:button-row> -->
							<button type="submit" value='<%= company.isSendPasswordResetLink() ? "send-password-reset-link" : "send-new-password" %>' />
						<!-- </aui:button-row> -->
					</c:otherwise>
				</c:choose>
			</c:when>
