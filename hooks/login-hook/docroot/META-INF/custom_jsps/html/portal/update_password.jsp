...
<%
	...
	if (referer.startsWith(themeDisplay.getPathMain()
...
<c:choose>
	<c:when test="<%=!themeDisplay.isSignedIn() && (ticket == null)%>">
		<div class="alert alert-warning">
			...
			<div>
				<aui:a href="<%=portletURL.toString()%>"
					label="request-a-new-password-reset-link"></aui:a>
			</div>
		</div>
	</c:when>
	<c:when
		test="<%=SessionErrors.contains(request,
							UserLockoutException.class.getName())%>">
		...
		<div class="login-block">
			<div>
				<div class="logo-main">
					...
					<img
						src="${themeDisplay.getPathThemeImages()}/showup-admin-images/logo-big.png" />
				</div>
				<div class="mini-form forgot-wrap">
					<form
						action='<%=themeDisplay.getPathMain()
							+ "/portal/update_password"%>'
						method="post" name="fm">


						<input name="p_l_id" type="hidden" value="<%=layout.getPlid()%>" />
						<input name="p_auth" type="hidden"
							value="<%=AuthTokenUtil.getToken(request)%>" /> <input
							name="doAsUserId" type="hidden"
							value="<%=themeDisplay.getDoAsUserId()%>" /> <input
							name="<%=Constants.CMD%>" type="hidden"
							value="<%=Constants.UPDATE%>" /> <input
							name="<%=WebKeys.REFERER%>" type="hidden"
							value="<%=referer%>" /> <input name="ticketKey" type="hidden"
							value="<%=ticketKey%>" />
				...
				<liferay-ui:message key="please-set-a-new-password" />
						...
						<c:if
							...
							<%
								UserPasswordException upe = (UserPasswordException) SessionErrors
													.get(request,
															UserPasswordException.class.getName());
							%>
							...
							<div class="alert alert-error">
								...
								<c:if
									...
									<liferay-ui:message
										key="passwords-may-not-be-changed-under-the-current-password-policy" />
								</c:if>
						...
						<div class="formRow">
							<div class="formInnerField">
								<label data-error="Please enter your new passsword"><span>New
										password</span></label> <input autoFocus="<%=true%>" name="password1"
									class="tetxBox" type="password" />
							</div>
						</div>

						<div class="formRow">
							<div class="formInnerField">
								<label data-error="Please re-enter your new passsword"><span>Re-enter
										password</span></label> <input name="password2" class="tetxBox"
									type="password" />
							</div>
						</div>
						<!-- 		</aui:fieldset>  -->
				...
				<aui:button type="submit" />
						...
						<button type="submit" class="button button-fluid button-primary">reset
							password</button>
