...
<% 
String aboutyoudeclareid = PortletProps.get("about.you.declaration");
String userknownid = PortletProps.get("User_exists_with_this_email_address_but_name_and/or_title_do_not_match.");
long groupid = themeDisplay.getLayout().getGroupId();
String passwordpolicy =  PortletProps.get("password.policy.message");
String marketingpreferences= PortletProps.get("Marketing.preferences");
%>
...
<div class="help-message"><liferay-ui:journal-article articleId='<%=PortletProps.get("password_help")%>' groupId="<%=groupid%>"  ></liferay-ui:journal-article></div>
          ...
          <form:input type="hidden" path="deviceDetails.blackBox2" id="blackBox2" value="" />
				...
				<% 
				String ipAddress = PortalUtil.getHttpServletRequest(renderRequest).getHeader("X-FORWARDED-FOR");
		                    if (ipAddress == null) {
		                      ipAddress = PortalUtil.getOriginalServletRequest(PortalUtil.getHttpServletRequest(renderRequest)).getRemoteAddr();
		                   }
		          %>
          <form:input type="hidden" path="deviceDetails.ipAddress"  value="<%=ipAddress%>"/>
            </form:form>
        </div>
    </section>
	        ...
	        // alert(occInput);
	          ...
	          if(postCode.length > 0 && tit.length<23){ 
	        	...
	        	//removeAddressIndex(addressindex);
	        	 var monthId=this.id;
		         var indexId = monthId.substring(5, 6);
	        	...
	        	var tit = $(".occupancy option:selected").text();
	 	        if (tit == "" && tit.length<23) {
	 	        	...
	 	        	$('.form-fourth-common').slideDown(function() {
		                ...
		                $(this).parents('.form-steps').addClass('open');
		               //. $('.address-note').remove();
		                $(this).find('.address-note').remove();
		                $("#bankname").focus();
		            });
	 	        }
	        	
	 	       $('.how-long-u').each(function(){
	        		var tYear = $(this).find('.time-year').val(),
	        			tMonth = $(this).find('.time-month').val();
	        		if(tYear == ""  || tMonth == ""){
	        			addressindex--;
	        			$('.form-fourth-common').slideUp();
	        			$(this).find('.form-inner-field').addClass('addRed');
	        			$(this).find('.time-month').val('');
	        		}else{
	        			$('.form-fourth-common').slideDown();
	        			
	        		}
	        		$('.how-long-u input').on('change', function(){
	        			$(this).parents('.how-long-u').find('.form-inner-field').removeClass('addRed');
	        		});
